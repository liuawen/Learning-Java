...
//		str = "23.23454789";
		...
		str = new Handleable() {
			...
			public String handleString(String num) {
				...
				if(index != -1){
					//截取到小数点后[wei]位
					temp = num.substring(0, index + 1 + wei);//截取[0,index+1+wei)部分
					//如果原数字的小数点总位数超过小数点后4位
					if(temp.length() < num.length()){
						//那就要考虑[wei+1]位的数字字符是在[0-4]还是[5-9]范围
						//取出[wei+1]位的数字字符
						char weiHouChar = num.charAt(index + 1 + wei);
						if(weiHouChar >= '5' && weiHouChar <= '9'){
							//取出[wei]位数字字符
							char weiChar = num.charAt(index + wei);
							//需要入一位
							weiChar = (char)(weiChar + 1);
							//截取到小数点后[wei-1]位 ,再拼接上地[wei]位数字字符
							temp = temp.substring(0, index + wei) + weiChar;
						}
					}
				}
				return temp;
...
public class Test {
	...
	public void g(int b, int[] c,String s){
		a = 1;
		b = 1;//修改形参b，局部变量，和成员变量b无关，因为b是基本数据类型，所以和实参b也无关
		c[0] = 1;
        s = "world";
	}
	public static void main(String[] args) {
